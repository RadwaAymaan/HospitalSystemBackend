...
public class ItemService(HMSBaseDbContext dbContext, IMapper mapper, ILogger<ItemService> logger, IUserContextService userContext) : IItemService
{
    ...
    public async Task<Result> DecreaseItemsQuantity(ICollection<ItemOrderRequestDto> orderedItems)
    {
        ...
        foreach (var item in items)
        {
            ...
            if (item.Stock < 0)
            {
                ...
                return Result.Error([$"{item.Name} does not have sufficient quantity available in stock"]);
            }
        }
